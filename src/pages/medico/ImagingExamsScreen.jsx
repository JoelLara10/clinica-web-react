...
export default function ImagingExamsScreen() {
  ...
  useEffect(() => {
    if (!idAtencion) {
      setErrorMessage('Selecciona un paciente antes de solicitar estudios de gabinete.');
      return;
    }
  ...
  const handleSubmit = async () => {
    if (selectedExams.length === 0) {
      window.alert('Selecciona al menos un estudio de gabinete.');
      return;
    }
    ...
    try {
      ...
      window.alert(error.response?.data?.error || 'No se pudieron solicitar los estudios');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={styles.page}>
      ...
      <section style={styles.mainCard}>
        <div style={styles.cardHeader}><MdOutlineScreenshotMonitor size={22} /><strong>Nueva Solicitud</strong></div>
        <div style={styles.cardBody}>
          ...
          {loadingCatalog ? <div style={styles.statusBox}>Cargando catálogo...</div> : exams.length === 0 ? <div style={styles.statusBox}>No hay estudios disponibles.</div> : (
          ...
          <label style={styles.fieldGroup}>
            ...
            <textarea style={styles.textArea} rows={4} value={observations} placeholder="Indicaciones para gabinete..." onChange={(event) => setObservations(event.target.value)} />
          </label>
      ...
      <footer style={styles.footer}><FiShield size={14} /><span>INEO v2.0 - Gabinete</span></footer>
    </div>
  );
}
